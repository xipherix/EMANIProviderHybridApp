...
void NIProviderClient::onRefreshMsg(const RefreshMsg& refreshMsg, const OmmProviderEvent& ommEvent)
{
	...
	{
		cout << endl << "NIProvier OnRereshMsg Handle: " << ommEvent.getHandle() << " Closure: " << ommEvent.getClosure() << endl;
		if (bDumpOutput)
...
void ConsumerClient::onRefreshMsg(const RefreshMsg& refreshMsg, const OmmConsumerEvent& ommEvent)
{
	try {
		...
		if (refreshMsg.getDomainType() == MMT_LOGIN)
		{
			cout << endl << "Login OnRereshMsg Handle: " << ommEvent.getHandle() << " Closure: " << (char *)ommEvent.getClosure() << endl;
			if (bDumpOutput)
				cout << refreshMsg << endl;
		...
		{
			cout << endl << "Market Price OnRereshMsg Handle: " << ommEvent.getHandle() << " Closure: " << (char *)ommEvent.getClosure() << endl;
			if (bDumpOutput)
				cout << refreshMsg << endl;
			...
			{ 
				...
				if (it != pItemList->end())
				{
					string pubItemName = (*it).first;
					UInt64 pubHandle = (*it).second.NiPubHandle;
					RefreshMsg refresh;
					cout << "Publish Refresh messge to Non Interactive provider service..." << endl;
...
void ConsumerClient::onUpdateMsg(const UpdateMsg& updateMsg, const OmmConsumerEvent& ommEvent)
{
	try{
		...
		{
			cout << endl << "Market Price OnUpdateMsg Handle: " << ommEvent.getHandle() << " Closure: " << (char *)ommEvent.getClosure() << endl;
			if (bDumpOutput)
				cout << updateMsg << endl;
